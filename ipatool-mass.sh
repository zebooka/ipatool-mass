...
while read L; do
    ...
    if find . -type f -iname "${B}_*_${V}.ipa" | grep -qz . ; then
        echo -e "\033[2;37m$(find . -type f -iname "${B}_*.ipa" | sort -r | head -n1 | sed -E 's#^\./##') \033[0;37m$S\033[0m"
    else
...
find . -type d -maxdepth 1 -mindepth 1 | while read D; do if [ `find "$D" -type f -iname '*.ipa' | wc -l` -gt 1 ]; then
    echo -e "\033[1;36m$(echo "$D" | sed -E 's|^\./||g')\n\033[2;37m$(find "$D" -type f -iname '*.ipa' | sort)\033[0m"
fi; done
